...
class AttBottleneckCSP(nn.Module):
    # CSP Bottleneck https://github.com/WongKinYiu/CrossStagePartialNetworks
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(AttBottleneckCSP, self).__init__()
        c_ = int(c2 * e)  # hidden channels

        # self.max_pool = nn.AdaptiveMaxPool2d((5,5))
        # self.cv5 = nn.Conv2d(c1, int(c1/2), 5, 1, 0, groups=g, bias=False)
        # self.ch_bnn = nn.BatchNorm2d(int(c1/2))

        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = nn.Conv2d(c1, c_, 1, 1, bias=False)
        self.cv3 = nn.Conv2d(c_, c_, 1, 1, bias=False)
        self.cv4 = AttConv(2 * c_, c2, 1, 1)
        self.bn = nn.BatchNorm2d(2 * c_)  # applied to cat(cv2, cv3)
        self.act = nn.LeakyReLU(0.1, inplace=True)
        self.sig =  nn.Sigmoid()
        self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
    ...
    def forward(self, x):
        ...
        y3 = self.cv3(y2)
        y4 = self.cv2(x)
        y_p = self.act(self.bn(torch.cat((y3, y4), dim=1)))
        last_c = self.cv4(y_p)
        sig_r = self.sig(last_c)

        # new_x = sig_r*x
        # new_x2 = self.max_pool(new_x)
        # new_x3 = self.ch_bnn(self.cv5(new_x2))

        return last_c, sig_r#, self.sig(new_x3)