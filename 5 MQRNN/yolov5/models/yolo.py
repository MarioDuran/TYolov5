...
from models.common import Conv, Bottleneck, SPP, DWConv, Focus, BottleneckCSP, Concat, NMS, autoShape, AttBottleneckCSP, \
    AttConv
...
class Detect(nn.Module):
    ...
    def forward(self, x, masksOP=None):
        ...
        for i in range(self.nl):
            ...
            cx = x[i].view(-1, self.sl, x[i].shape[1], x[i].shape[2], x[i].shape[3]).permute(0, 2, 1, 3, 4).contiguous()
            masksOP = torch.nn.functional.interpolate(masksOP, (x[i].shape[2], x[i].shape[3]))
            xo = self.qrnn[i](cx)
            xo = xo.permute(0, 2, 1, 3, 4)

            # cx = x[i].view(-1, self.sl, x[i].shape[1], x[i].shape[2], x[i].shape[3])
            # xo, xhc, att_maps = self.convlstms[i](cx, masksOP)
            cx = xo.contiguous().view(-1, x[i].shape[1], x[i].shape[2], x[i].shape[3])
...
class QRNN3DLayer(nn.Module):
    def __init__(self, in_channels, hidden_channels, act='relu'):
        ...
        self.conv = nn.Conv3d(in_channels, 2 * hidden_channels, (2, 3, 3), padding=(0, 1, 1))
